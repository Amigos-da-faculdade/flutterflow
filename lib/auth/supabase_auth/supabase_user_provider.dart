...
class ProjectSkyLabSupabaseUser extends BaseAuthUser {
  ProjectSkyLabSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> projectSkyLabSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ProjectSkyLabSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
